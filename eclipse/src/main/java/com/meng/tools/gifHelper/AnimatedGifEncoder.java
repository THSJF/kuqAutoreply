...
public class AnimatedGifEncoder {
    ...
    protected final int palSize = 7; // color table size (bits-1)
    ...
    protected int findClosest(Color c) {
        ...
        int minpos = 0;
        int dmin = 0x1000000;//256 * 256 * 256
        int len = colorTab.length;
        for (int i = 0; i < len; ) {
            int dr = c.getRed() - (colorTab[i++] & 0xff);
            int dg = c.getGreen() - (colorTab[i++] & 0xff);
            int db = c.getBlue() - (colorTab[i] & 0xff);
