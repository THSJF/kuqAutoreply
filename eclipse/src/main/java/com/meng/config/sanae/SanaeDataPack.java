...
public class SanaeDataPack {
	...
	public static SanaeDataPack encode(int opCode, SanaeDataPack SanaeDataPack) {
		return new SanaeDataPack(opCode, SanaeDataPack);
	}
	...
	private SanaeDataPack(int opCode, SanaeDataPack SanaeDataPack) {
		//length(4) headLength(2) version(2) time(8) target/from(8) opCode(4)
		writeByteDataIntoArray(BitConverter.getBytes(0));
		writeByteDataIntoArray(BitConverter.getBytes(headLength));
		writeByteDataIntoArray(BitConverter.getBytes(SanaeDataPack.getVersion()));
		writeByteDataIntoArray(BitConverter.getBytes(SanaeDataPack.getTimeStamp()));
		writeByteDataIntoArray(BitConverter.getBytes(SanaeDataPack.getTarget()));
		writeByteDataIntoArray(BitConverter.getBytes(opCode));
	}   
	...
	private void writeByteDataIntoArray(byte... bs) {
		for (byte b:bs) {
			data.add(b);
			++dataPointer;
		}
